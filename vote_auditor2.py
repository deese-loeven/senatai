...
class VoteAuditor:
    ...
    def get_user(self):
        """Get the current user or prompt for login."""
    ...
    def get_user_policap(self):
        """Get the current policap balance for the user."""
        self.cursor.execute("SELECT policap FROM users WHERE username = ?", (self.user,))
    ...
    def update_user_policap(self, amount):
        """Update the user's policap balance."""
        ...
        self.cursor.execute("UPDATE users SET policap = ? WHERE username = ?", 
                           (new_policap, self.user))
        self.conn.commit()
        return True
    ...
    def record_user_feedback(self, prediction_id, agrees):
        """Record the user's feedback on a prediction."""
        ...
        self.cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (prediction_id, self.user, agrees, timestamp))
    ...
    def run_audit(self):
        ...
        for i, prediction in enumerate(predictions, 1):
            ...
            if user_input in ['yes', 'no']:
                # Check if user has enough policap
