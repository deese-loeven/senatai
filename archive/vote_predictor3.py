...
def get_user_answers(Senatair_id, db_name="survey_data.db"):
    try:
        conn = sqlite3.connect(db_name)
        cursor = conn.cursor()

        # Retrieve Senatair answers with their questions and types
        cursor.execute("""
        ...
        print(f"Error retrieving Senatair answers: {e}")
        return []

def predict_vote(bill_text, user_answers):
    # Simple prediction: look for keywords in bill text and match to Senatair answers
    relevant_answers = []
    ...
    # Score the bill based on Senatair answers
    total_score = 0
    count = 0
...
def store_prediction(bill_text, Senatair_id, predicted_vote, explanation, db_name="survey_data.db"):
    try:
        conn = sqlite3.connect(db_name)
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO predicted_votes (bill_text, Senatair_id, predicted_vote, explanation, timestamp) VALUES (?, ?, ?, ?, ?)",
            (bill_text, Senatair_id, predicted_vote, explanation, datetime.now())
        )
        conn.commit()
        conn.close()
    except Exception as e:
...
def get_policap_balance(Senatair_id, db_name="survey_data.db"):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.execute("SELECT tokens FROM Senatairs WHERE id = ?", (Senatair_id,))
    result = cursor.fetchone()
    conn.close()
    return result[0] if result else 0

def spend_policap(Senatair_id, amount, db_name="survey_data.db"):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.execute("UPDATE Senatairs SET tokens = tokens - ? WHERE id = ?", (amount, Senatair_id))
    conn.commit()
    conn.close()
...
def main():
    ...
    # Get Senatair answers
    user_answers = get_user_answers(Senatair_id)
    if not user_answers:
        print("No Senatair answers found. Please complete some surveys first.")
        return

    # Check policap balance
    policap_balance = get_policap_balance(Senatair_id)
    print(f"Your policap balance: {policap_balance}")

    # Let Senatair choose to vote directly or delegate
    for bill in legislation:
        ...
        store_prediction(bill, Senatair_id, vote, explanation)
