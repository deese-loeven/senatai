...
class SurveyWindow:
    def __init__(self, master, db_name="survey_data.db", Senatair_id=""):
        self.master = master
        self.Senatair_id = Senatair_id
        self.conn = sqlite3.connect(db_name)
        self.cursor = self.conn.cursor()
        
        # Window setup
        master.title(f"Survey Session - {Senatair_id}")
    ...
    def load_next_question(self):
        """Get a new question the Senatair hasn't answered"""
        self.cursor.execute("""
...
def launch_survey_window(Senatair_id):
    root = tk.Tk()
    SurveyWindow(root, Senatair_id=Senatair_id)
    root.mainloop()
