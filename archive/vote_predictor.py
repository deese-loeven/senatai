...
def init_db():
    ...
    # User Voting Power
    c.execute('''CREATE TABLE IF NOT EXISTS Senatairs
                 (Senatair_id TEXT PRIMARY KEY,
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS votes
                 (vote_id INTEGER PRIMARY KEY AUTOINCREMENT,
                  ...
                  FOREIGN KEY(Senatair_id) REFERENCES Senatairs(Senatair_id))''')
...
class TokenSystem:
    ...
    def earn_tokens(self, Senatair_id, amount=1):
        """Reward for answering surveys"""
        c = self.conn.cursor()
        c.execute('''INSERT OR IGNORE INTO Senatairs (Senatair_id, tokens) VALUES (?, 10)''', (Senatair_id,))
        c.execute('''UPDATE Senatairs SET tokens = tokens + ? WHERE Senatair_id = ?''', (amount, Senatair_id))
        self.conn.commit()
        return c.execute('''SELECT tokens FROM Senatairs WHERE Senatair_id = ?''', (Senatair_id,)).fetchone()[0]
    
    def spend_tokens(self, Senatair_id, bill_id, position, amount):
        """Spend tokens to influence legislation"""
        if amount <= 0:
            return False
        
        c = self.conn.cursor()
        c.execute('''SELECT tokens FROM Senatairs WHERE Senatair_id = ?''', (Senatair_id,))
        balance = c.fetchone()[0]
        
        if balance >= amount:
            c.execute('''UPDATE Senatairs SET tokens = tokens - ?, last_vote = ? WHERE Senatair_id = ?''',
                     (amount, datetime.now(), Senatair_id))
            c.execute('''INSERT INTO votes (bill_id, Senatair_id, position, tokens_spent, timestamp)
                         VALUES (?, ?, ?, ?, ?)''',
                         (bill_id, Senatair_id, position, amount, datetime.now()))
...
class PoliticalPredictor:
    ...
    def answer_survey(self, Senatair_id, question_id, answer):
        """Answer a question and earn tokens"""
        # Store answer (using your existing question system)
        self.token_system.earn_tokens(Senatair_id)
        # Add to training data for model updates
        self._add_to_training_data(question_id, answer, Senatair_id)
    
    def vote_on_bill(self, Senatair_id, bill_id, position, tokens_to_spend):
        """Spend tokens to influence a bill"""
        return self.token_system.spend_tokens(Senatair_id, bill_id, position, tokens_to_spend)
    ...
    def _add_to_training_data(self, question_id, answer, Senatair_id):
        """Add survey answers to your model training data"""
        conn = sqlite3.connect('political_predictor.db')
        c = conn.cursor()
        c.execute('''INSERT INTO survey_answers 
                     (question_id, Senatair_id, answer, timestamp)
                     VALUES (?, ?, ?, ?)''',
                     (question_id, Senatair_id, answer, datetime.now()))
        conn.commit()
        conn.close()
