...
class VotePredictor:
    ...
    def predict_vote_simple(self, bill_text, user_answers):
        # Current prediction logic (based on average score)
        relevant_answers = []
    ...
    def predict_vote_alternative(self, bill_text, user_answers):
        # Dummy alternative model: always votes "yes" if any positive sentiment, otherwise "no"
        positive_count = 0
        total_count = 0
        bill_text_lower = bill_text.lower()
        for answer in user_answers:
