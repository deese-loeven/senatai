...
# THEMES.py
THEMES = {
    "seniors": ["senior", "seniors", "elderly", "retirement"],
    "small_business": ["small business", "entrepreneur", "startup", "businesses"],
    "taxation": ["tax", "taxes", "taxation", "fiscal"],
    "gun_rights": ["gun", "guns", "firearm", "firearms", "second amendment"],
    "climate_change": ["climate", "environment", "global warming", "carbon"],
    "indigenous_communities": ["indigenous", "native", "aboriginal", "first nations"],
    "free_speech": ["free speech", "censorship", "expression"],
    "healthcare": ["healthcare", "health care", "medical", "hospitals"],
    "provincial_autonomy": ["provincial autonomy", "federalism", "province", "territory"],
    "economic_growth": ["economic growth", "economy", "GDP", "development"],
    "child_care": ["child care", "daycare", "early childhood", "kids"],
    "oil_workers": ["oil workers", "energy sector", "petroleum", "oil and gas"],
    "urban_renters": ["urban renters", "rent", "housing affordability", "city dwellers"],
    "general":["general"]
}
...
class SurveySystem:
    ...
    def ask_question(self, Senatair_id):
        ...
        # Award 1 policap
        self.cursor.execute("UPDATE Senatairs SET tokens = tokens + 1, last_active = ? WHERE id = ?",
                            (datetime.now(), Senatair_id))
        self.conn.commit()
        print("Question asked and answer recorded!")

# Simulate a Senatair login and interaction
if __name__ == "__main__":
    system = SurveySystem()
    Senatair_id = "user123"
    # Ensure Senatair exists
    ...
    system.ask_question(Senatair_id)
