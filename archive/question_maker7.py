#!/usr/bin/env python3
import random
from faker import Faker  # For generating natural language variations
...
def generate_theme_question():
    ...
    templates = [
        f"Should Canada prioritize {theme} even if it means less focus on {random.choice(THEMES[random.choice([k for k in THEMES if k != theme_category])])}?",
        ...
        f"Is protecting {theme} worth limiting {random.choice(THEMES[random.choice([k for k in THEMES if k != theme_category])])}?"
    ]
