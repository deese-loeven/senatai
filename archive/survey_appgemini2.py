...
class SurveySystem:
    ...
    def reset_daily_count(self, Senatair_id):
        self.cursor.execute("SELECT last_reset FROM Users WHERE Senatair_id = ?", (Senatair_id,)) #Corrected here.
        last_reset = self.cursor.fetchone()[0]
        today = date.today().isoformat()

        if last_reset != today:
            self.cursor.execute(
                "UPDATE Users SET daily_answer_count = 0, last_reset = ? WHERE Senatair_id = ?", #Corrected here.
                (today, Senatair_id)
            )
            self.conn.commit()
    ...
    def ask_question(self, Senatair_id):
        ...
        self.cursor.execute("SELECT daily_answer_count FROM Users WHERE Senatair_id = ?", (Senatair_id,)) #Corrected here.
        ...
        self.cursor.execute(
            "UPDATE Users SET tokens = tokens + ?, daily_answer_count = daily_answer_count + 1, last_active = ? WHERE Senatair_id = ?", #Corrected here.
...
if __name__ == "__main__":
    system = SurveySystem()
    Senatair_id = "user123"
    system.cursor.execute("INSERT OR IGNORE INTO Users (Senatair_id, last_active) VALUES (?, ?)", #Corrected here.
    ...
    system.cursor.execute("SELECT tokens, daily_answer_count FROM Users WHERE Senatair_id = ?", (Senatair_id,)) #Corrected here.
    total_policap, daily_count = system.cursor.fetchone()
    print(f"Total policap: {total_policap}")
    print(f"Answers today: {daily_count}")
if __name__ == "__main__":
    system = SurveySystem()
    Senatair_id = input("Enter your Senatair ID: ")
    system.cursor.execute("INSERT OR IGNORE INTO Users (Senatair_id, last_active) VALUES (?, ?)",
                        (Senatair_id, datetime.now()))
    system.conn.commit()
    print(f"Logged in as {Senatair_id}")
