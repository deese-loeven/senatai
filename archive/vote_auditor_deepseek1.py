...
class VoteAuditor:
    ...
    def get_user(self):
        """Authenticate or register Senatair and load their policap balance"""
        while True:
            Senatair_id = input("Enter your Senatair ID: ").strip()
            if not 3 <= len(Senatair_id) <= 30:
                print("User ID must be 3-30 characters")
                continue

            self.cursor.execute(
                "SELECT username, tokens FROM Users WHERE Senatair_id = ?",
                (Senatair_id,)
            )
            ...
            if Senatair:
                ...
                self.current_user = {
                    'id': Senatair_id,
                    'username': Senatair['username'],
                    'policaps': Senatair['tokens']
                }
                return True
            else:
                ...
                try:
                    self.cursor.execute(
                        """INSERT INTO Users 
                        (Senatair_id, username, email, tokens) 
                        VALUES (?, ?, ?, ?)""",
                        (Senatair_id, username, email, 10.0)  # Starting with 10 policaps
                    )
                    self.conn.commit()
                    ...
                    print("That Senatair ID or username already exists. Try again.")
                    continue
    ...
    def get_user_decision(self):
        """Get and validate Senatair's decision on a prediction"""
        while True:
            decision = input(
    ...
    def process_audit(self, prediction_id, decision, amount):
        """Record the Senatair's audit decision"""
        ...
        try:
            ...
            self.cursor.execute(
                """INSERT INTO Audits 
                (prediction_id, Senatair_id, audit_action, token) 
                VALUES (?, ?, ?, ?)""",
            ...
            self.cursor.execute(
                "UPDATE Users SET tokens = ? WHERE Senatair_id = ?",
...
if __name__ == "__main__":
    ...
    try:
        if auditor.get_user():  # Try to log the Senatair in
