...
def answer_questions():
    ...
    answered_ids = [a.question_id for a in 
                   Answer.query.filter_by(Senatair_id=current_user.id).all()]
...
def audit_predictions():
    # Get bills with predictions but no final vote from Senatair
    voted_bills = [v.bill_id for v in 
                  UserVote.query.filter_by(Senatair_id=current_user.id).all()]
...
def update_bill_statistics(bill_id):
    ...
    if votes:
        ...
        # Calculate audit rate (percentage of Senatairs who voted)
