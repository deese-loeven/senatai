...
# Enhanced query: Join billtext for full text if summary empty; limit recent/non-empty
cur.execute("""
    SELECT 
        b.id,
        b.short_title_en,
        COALESCE(b.summary_en, b.long_title) AS text_summary,  -- Fallback to long_title
        bt.full_text  -- Full bill text from billtext
    FROM bills_bill b
    LEFT JOIN bills_billtext bt ON b.id = bt.bill_id AND bt.language = 'en'  -- English full text
    WHERE b.summary_en IS NOT NULL OR b.long_title IS NOT NULL OR bt.full_text IS NOT NULL
    ORDER BY b.latest_debate_date DESC NULLS LAST  -- Recent first, push blanks last
    LIMIT 5
""")
rows = cur.fetchall()
...
for row in rows:
    ...
    # Combine: summary/long_title + full_text (truncate if too long)
    text = f"{short_title_en or ''} {text_summary or ''} {full_text[:2000] or ''}".strip()  # Limit full_text for perf
    if not text:
    ...
    # Extract entities (NER: ORG, GPE, LAW, etc.)
    ...
    # Extract keywords: nouns, proper nouns, adjectives (lowercase, unique)
    ...
    unique_keywords = [word for word, _ in bill_counter.most_common(10)]  # Top 10
