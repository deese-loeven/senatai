...
class VoteAuditor:
    ...
    def audit_predictions(self, user_id=None, bill_text=None):
    ...
    def _calculate_accuracy(self, prediction):
        """Compare prediction with actual user votes"""
        # Get user's actual vote on similar questions
        self.cursor.execute("""
        ...
        """, (prediction['user_id'], f"%{prediction['bill_text']}%"))
...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        audits = auditor.audit_predictions(user_id=user)
    elif choice == "3":
