...
class SurveySystem:
    ...
    def reset_daily_count(self, id):
        ...
        if last_reset != today:
            self.cursor.execute(
                "UPDATE Senatairs SET daily_answer_count = 0, last_reset = ? WHERE id = ?",
                (today, id)
            )
            self.conn.commit()
            print(f"Daily count reset for Senatair {id}")
    ...
    def ask_question(self, id):
        ...
        self.cursor.execute(
            "UPDATE Senatairs SET policaps = policaps + ?, daily_answer_count = daily_answer_count + 1, last_active = ? WHERE id = ?",
...
def get_id(system):
    while True:
        id = input("Please enter your unique Senatair ID (3-30 characters): ").strip()
        if id:
            if 3 <= len(id) <= 30:
                return id
            else:
...
if __name__ == "__main__":
    ...
    # Check if Senatair already exists
    system.cursor.execute("SELECT id FROM Senatairs WHERE id = ?", (id,))
    ...
    if existing_user:
        ...
        print(f"New Senatair created: {id}. Welcome!")
    ...
    system.cursor.execute("SELECT policaps, daily_answer_count FROM Senatairs WHERE id = ?", (id,))
