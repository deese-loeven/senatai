...
class SurveySystem:
    ...
    def select_question_maker(self):
        """Let user select which question maker to use"""
    ...
    def ask_question(self, id):
        ...
        # Update user tokens
        self.cursor.execute("""
            UPDATE users SET policaps = policaps + 1, last_active = ?
            WHERE id = ?
        """, (datetime.now(), id))
...
def get_id():
    ...
    while True:
        id = input("\nEnter your user ID (3-20 chars): ").strip()
        if 3 <= len(id) <= 20:
            return id
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == "1":  # Answer questions
            ...
            system.cursor.execute("""
                SELECT policaps FROM users WHERE id = ?
            """, (id,))
